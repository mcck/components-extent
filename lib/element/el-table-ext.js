  ...
  let obj = {
    ...
    methods: {
      ...
      getCondition(queryParam, param2) {
        ...
        param.condition = con.length ? con : [];
        return param;
      },
      ...
      getTableList() {
        ...
        if (self.getTableListBefore) {
          let res = self.getTableListBefore(param);
          param = res == undefined ? param : res;
        }
        if(!param.condition?.length){
